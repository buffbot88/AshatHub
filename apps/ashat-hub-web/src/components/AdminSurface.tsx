...
type CodingAgentRepoStatus = { ok?: boolean; repo?: string; branch?: string; direction?: string; local_sha?: string; remote_sha?: string; ahead?: number; behind?: number; peers?: string[]; message?: string; error?: string };
...
function TelemetryTab({ telemetry, busy, onAction, onUpdate, onPush, status }: { telemetry: AdminTelemetry | null; busy: boolean; onAction: (action: 'refresh' | 'clear') => void; onUpdate: () => void; onPush: () => void; status: RepoStatus | null }) {
  return <div className="adm-telemetry-page">
    <div className="adm-repo-status"><div><strong>Coding Agents</strong><span className="adm-repo-subtitle">AshatCodingAgent · Omega master · Beta/Delta slave</span></div><div className="adm-repo-commit"><span>Live / GitHub</span><code>{codingAgentStatus(status).local_sha ? `${(codingAgentStatus(status).local_sha || '').slice(0, 8)} / ${(codingAgentStatus(status).remote_sha || '').slice(0, 8)}` : codingAgentStatus(status).error || 'Loading…'}</code><small className={`adm-agent-state adm-agent-${codingAgentStatus(status).direction || 'unknown'}`}>{codingAgentStatus(status).direction || 'status unavailable'}</small></div></div>
    ...
    <section className="adm-hotfix-card"><div><span className="eyebrow">Push Hotfixes</span><h3>Publish verified hotfixes to GitHub</h3><p>Push committed Omega hotfixes from the master AshatCodingAgent repository to GitHub for the ecosystem release.</p></div><button type="button" className="secondary-button" onClick={onPush} disabled={busy}>{busy ? 'Pushing…' : 'Push Hotfixes to GitHub'}</button></section>
