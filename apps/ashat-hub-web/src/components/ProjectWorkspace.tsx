...
type Deployment = { ok: boolean; project_id: string; status: string; url?: string | null; deployment_id?: string | null; file_count?: number | null };
...
export function ProjectWorkspace({ user, onTaskChange }: { user: User | null; onTaskChange: (task: TaskFrameData | null) => void }) {
  ...
  async function deployProject() {
    ...
    setPreviewBusy(true); setError(null);
    try {
      const data = await request<Deployment>(`${RUST_API}/galileo/deploy`, { method: 'POST', body: JSON.stringify({ project_id: projectId }) });
      setDeployment(data);
