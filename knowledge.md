...
- **Build backend fallback (BYO or server BrainStem)**: `runBuildStream` builds browser-direct with a valid BYO config (`getByoConfig()` — endpoint + key), but with **no BYO key it falls back to the server's default BrainStem stream** via `agent.js`'s `serverChatStream()` — a `POST /api/chat/stream/` (CSRF header from the meta tag, same SSE `meta`/`delta`/`done` protocol the chat text path uses), with `event: error` surfaced as a thrown Error. `generateFilesInChat` no longer hard-requires `localStorage["ashat.api"]`, so users on the default BrainStem system get files generated server-side (keys stay on the device only when BYO is used). The build's `meta` event also keeps the status pill in sync with the resolved serving model.
