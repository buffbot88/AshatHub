...
Email verification is currently disabled in the production config because no outbound mail transport is configured. Registration still requires PHP `FILTER_VALIDATE_EMAIL` syntax validation, but does not prove mailbox ownership. To re-enable verification after configuring a real mail transport, set `EMAIL_VERIFICATION_ENABLED` to `true` in `config/server_config.json`.
