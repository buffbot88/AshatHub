...
(function () {
  ...
  var currentMode = (function () {
    try { return localStorage.getItem(MODE_KEY) || 'chat'; } catch (_) { return 'chat'; }
  })();

  var modeTabs = document.querySelectorAll('.chat-mode-tab');

  var MODE_PROMPTS = {
    chat: {
      name: 'Chat',
      placeholder: 'Ask me anything...',
      subtitle: 'General conversation with AI',
      prompt: 'You are Ashat, a helpful AI assistant at ASHAT Hub. Be conversational, friendly, and helpful. Answer questions directly and clearly. You can discuss any topic, help with coding, brainstorm ideas, or just have a conversation. Keep responses concise but thorough.'
    },
    brainstorm: {
      name: 'Brainstorm',
    ...
    build: {
      ...
      subtitle: 'Generate code from specs',
      prompt: 'You are Ashat, an expert software developer at ASHAT Hub. Your role is to generate clean, production-ready code. When the user describes what they want, output complete, working code files. Always present the complete file structure first, then give ONE code block per file with the exact filename above each block. Write modern, best-practice code. After generating code, ask if they want to save it to their Project Files.'
    }
  };

  function getModePrompt(mode) {
    return MODE_PROMPTS[mode] ? MODE_PROMPTS[mode].prompt : SYSTEM_PROMPT.content;
  }

  function getModePlaceholder(mode) {
    return MODE_PROMPTS[mode] ? MODE_PROMPTS[mode].placeholder : 'Describe your project idea...';
  }

  function getModeSubtitle(mode) {
    return MODE_PROMPTS[mode] ? MODE_PROMPTS[mode].subtitle : '';
  }

  function switchMode(mode) {
    if (!MODE_PROMPTS[mode]) return;
    currentMode = mode;
    ...
    if (input) {
      input.placeholder = getModePlaceholder(mode);
    }

    // Update subtitle
    var subtitleEl = document.querySelector('#chat-header-info p');
    if (subtitleEl) {
    ...
    if (conv && conv.messages && conv.messages.length > 0) {
      var newPrompt = getModePrompt(mode);
      // Find and replace the system prompt
      for (var i = 0; i < conv.messages.length; i++) {
        if (conv.messages[i].role === 'system' && !conv.messages[i].content.startsWith('[Project Context') && !conv.messages[i].content.startsWith('[Earlier conversation summary') && !conv.messages[i].content.startsWith('[Prior Conversations')) {
          conv.messages[i].content = newPrompt;
          break;
        }
      }
      saveConversations();
...
// ── Constants ────────────────────────────────────────────────────
  ...
  var SYSTEM_PROMPT = {
    ...
    content: [
      ...
      '',
      'ONE POSTURE PER MESSAGE — never mix two in a single reply:',
      '  MODE A — GATHERING: ask focused questions, discuss tradeoffs, refine details.',
      '    Do NOT draft, preview, or summarize the spec here — not even a short version.',
      '  MODE B — DELIVERING: output the COMPLETE spec, then STOP. No follow-up',
      '    questions, no "would you like to refine?" in the same message as the spec.',
      '',
      'Emit the spec ONLY when you have enough concrete detail to fill every section.',
      'If any section would be vague or guessed, stay in MODE A and ask for the missing detail.',
      '',
      ...
      '',
      'When you switch to MODE B, output a Markdown',
  ...
  var DEFAULT_MODEL_LABEL = 'LFM2.5 450M VL';
  var BACKEND_LABELS = { brainstem: 'BrainStem Neural Host (Omega)', local: 'Local VL' };
  ...
  /** The model actually in use: local VL in Chat mode, Omega elsewhere. */
  function backendModelLabel() {
    if (currentMode === 'chat') return DEFAULT_MODEL_LABEL;
    return 'LFM2.5 1.2B Instruct (Omega)';
  }
  ...
  function appendMessageBubble(role, content, msg) {
    ...
    // User bubbles with an attached image render the thumbnail above text.
    var imageHtml = (role === 'user' && msg && msg.image)
      ? '<img class="chat-image" src="' + msg.image + '" style="max-width:220px;max-height:160px;border-radius:8px;border:1px solid var(--gold-line);margin-bottom:6px;display:block;">'
      : '';
  ...
  async function generateFilesInChat(spec) {
    // Server-side build pipeline (Omega, chain slot) — SSE progress.
    setBackendStatus('checking');
    var status = appendGenStatusBubble('Generating project files…');
    try {
      var response = await fetch('/api/build/pipeline/', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ spec: spec, plan: '', language: '' }),
      });
      if (!response.ok || !response.body) {
        throw new Error('Build pipeline request failed (HTTP ' + response.status + ')');
      }
      ...
      while (true) {
        ...
        for (var k = 0; k < parts.length; k++) {
          ...
          if (eventType === 'progress' && parsed.message && status) {
            status.textContent = parsed.message;
          } else if (eventType === 'error') {
            buildError = parsed.message || 'Build failed.';
      ...
      if (!files || !files.length) {
        throw new Error('The pipeline returned no files. Try again or simplify the spec.');
      }

      loadFileTree();
      var savedCount = (done && done.saved) || files.length;
      status.className = 'gen-status-bubble ok';
      status.textContent = '✅ ' + savedCount + ' file(s) generated into your Project Files.';
      if (done && done.issues && done.issues.length) {
        status.textContent += ' — ' + done.issues.length + ' validation note(s).';
      }
      ashatToast('Generated ' + savedCount + ' file(s) into your Project Files.', 'ok');
    } catch (err) {
      status.className = 'gen-status-bubble err';
  ...
  // ══════════════════════════════════════════════════════════════════
  //  BRAINSTORM (idea → Spec.md + Build.md via Omega chain slot)
  // ══════════════════════════════════════════════════════════════════
  ...
  async function runBrainstorm(idea) {
    ...
    try {
      ...
      if (!response.ok || !response.body) {
        throw new Error('Brainstorm request failed (HTTP ' + response.status + ')');
      }
      ...
      while (true) {
        ...
        for (var k = 0; k < parts.length; k++) {
          ...
          var parsed;
          try { parsed = JSON.parse(data); } catch (_) { continue; }
          if (eventType === 'done') done = parsed;
          else if (eventType === 'error') err = parsed.message || 'Brainstorm failed.';
          else if (eventType === 'progress' && parsed.message && status) status.textContent = parsed.message;
        }
      }
      if (err) throw new Error(err);
      if (!done || !done.ok) throw new Error('Brainstorm did not complete.');

      var conv = getActiveConversation();
      if (!conv) return;
      conv.messages.push({
        role: 'assistant',
        content: '✅ **Brainstorm complete.** Wrote **Spec.md** + **Build.md** into your Project Files (chain slot: Omega). The Build tab is now unlocked.\n\n---\n\n### Spec.md\n\n' + done.spec + '\n\n---\n\n### Build.md\n\n' + done.build,
      });
      touchConversation();
      renderMessages();
      if (status) {
        status.className = 'gen-status-bubble ok';
        status.textContent = '✅ Spec.md + Build.md written to your Project Files.';
      }
      ashatToast('Spec.md + Build.md written to your Project Files.', 'ok');
      refreshBuildTab();
    } catch (err2) {
      if (status) {
        status.className = 'gen-status-bubble err';
        status.textContent = '⚠ ' + (err2 && err2.message ? err2.message : 'Brainstorm failed.');
      }
      var conv = getActiveConversation();
      if (conv) {
        conv.messages.push({
          role: 'assistant',
          content: '⚠ **Brainstorm failed:** ' + (err2 && err2.message ? err2.message : 'unknown error'),
        });
        touchConversation();
        renderMessages();
      }
  ...
  function refreshBuildTab() {
    var tab = document.getElementById('build-mode-tab');
    if (!tab) return;
    var locked = !buildUnlocked();
    tab.classList.toggle('mode-locked', locked);
    var label = tab.querySelector('.mode-label');
    if (label) label.textContent = locked ? 'Build 🔒' : 'Build';
    var desc = tab.querySelector('.mode-desc');
    if (desc) desc.textContent = locked ? 'Needs Spec.md + Build.md' : 'Generate Code';
  }

  // ══════════════════════════════════════════════════════════════════
  //  IMAGE ATTACH (drag-drop / paste → local VL image-to-text)
  // ══════════════════════════════════════════════════════════════════
  ...
  function readImageFile(file) {
    if (file.size > 15 * 1024 * 1024) {
      ashatToast('Image too large — 15 MB max.', 'err');
      return;
    }
    var reader = new FileReader();
    reader.onload = function () {
      // Downscale before attaching: keeps localStorage history small
      // (full dataURLs blow the 5MB quota) and cuts vision-token count
      // (fewer mmproj tiles = faster on a single core).
      var img = new Image();
      img.onload = function () {
        var MAX = 1024;
        ...
        if (w > MAX || h > MAX) {
          ...
          attachImage(canvas.toDataURL('image/jpeg', 0.85), file.name);
        } else {
          attachImage(reader.result, file.name);
        }
      };
      img.onerror = function () { ashatToast('Could not read image.', 'err'); };
  ...
  function attachImage(dataUrl, name) {
    if (currentMode !== 'chat') {
      ashatToast('Images work in Chat mode (local 450M VL).', 'warn');
      return;
    }
    pendingImage = { url: dataUrl, name: name };
    var chip = document.getElementById('chat-image-chip');
    if (!chip) return;
    chip.innerHTML = '';
    var img = document.createElement('img');
    img.src = dataUrl;
    img.style.cssText = 'max-height:56px;border-radius:8px;border:1px solid var(--gold-line);';
    img.title = name || 'attached image';
    var btn = document.createElement('button');
    btn.type = 'button';
    btn.textContent = '×';
    btn.style.cssText = 'margin-left:6px;font-size:13px;color:var(--gold-err);background:none;border:none;cursor:pointer;';
  ...
  /** Wire drop + paste image handling onto the chat input. */
  function bindImageInput() {
    var chatForm = document.getElementById('chat-form');
    if (chatForm) {
      ['dragover', 'dragenter'].forEach(function (evt) {
        chatForm.addEventListener(evt, function (e) { e.preventDefault(); });
      });
      chatForm.addEventListener('drop', function (e) {
        e.preventDefault();
        var f = e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files[0];
        if (f && f.type && f.type.indexOf('image/') === 0) readImageFile(f);
      });
    }
    if (input) {
      input.addEventListener('paste', function (e) {
        ...
        for (var i = 0; i < items.length; i++) {
          if (items[i].type && items[i].type.indexOf('image/') === 0) {
            ...
            if (f) { readImageFile(f); e.preventDefault(); }
            break;
          }
        }
      });
  ...
  async function tryStream(body, headers) {
    ...
    try {
      ...
      setBackendStatus('checking');
      // Chat goes through the intent router (/api/ai/ → alpha-server):
      // model=local in Chat mode forces the pooled 450M VL; Brainstorm
      // and Build modes route to Omega via the same gateway.
      var response = await fetch('/api/ai/v1/chat/completions', {
        method: 'POST',
        headers: headers,
      ...
      if (!response.ok) {
        var errMsg = (data && data.error && (data.error.message || data.error)) || 'Request failed (HTTP ' + response.status + ')';
        streamError = typeof errMsg === 'string' ? errMsg : JSON.stringify(errMsg);
      } else {
        var delta0 = data.choices && data.choices[0] && data.choices[0].delta;
        var msg0   = data.choices && data.choices[0] && data.choices[0].message;
        var chunk = (delta0 && (delta0.content || delta0.reasoning_content || delta0.reasoning)) || (msg0 && msg0.content) || '';
        if (chunk) {
          fullContent += chunk;
          if (delta0 && (delta0.reasoning_content || delta0.reasoning)) {
            if (bubble.thinkingLabel) bubble.thinkingLabel.textContent = 'Thinking...';
            bubble.thinkingContent.textContent = delta0.reasoning_content || delta0.reasoning || '';
          } else {
      ...
      if (streamError) {
        ...
        if (streamError.indexOf('no_backend') !== -1 || streamError.indexOf('No AI backend') !== -1) {
          diag = '\n\n📋 **How to fix:** Ask an admin to configure the BrainStem host.';
        }
  ...
  async function tryNonStream(body, headers) {
    ...
    try {
      ...
      bubble.thinkingContent.textContent = '⚠ Could not reach the AI backend.\n\nMake sure an admin has configured the BrainStem host (Admin → Settings), then try again.';
  ...
  async function sendMessage(userText) {
    ...
    // Add user message (with attached image, if any)
    var userImage = pendingImage ? pendingImage.url : null;
    conv.messages.push({ role: 'user', content: userText, image: userImage });
    touchConversation();

    // Clear input, render user bubble
    input.value = '';
    autoResizeInput();
    appendMessageBubble('user', userText, userImage ? { image: userImage } : null);
    setStreamingState(true);
    showTypingIndicator();

    // Brainstorm mode: idea → Spec.md + Build.md via Omega (the chain).
    if (currentMode === 'brainstorm') {
      await runBrainstorm(userText);
      return;
    }

    // Build mode is gated: Spec.md + Build.md must exist on disk.
    if (currentMode === 'build' && !buildUnlocked()) {
      hideTypingIndicator();
      conv.messages.push({ role: 'assistant', content: '🔒 **Build is locked.** Brainstorm your idea first — that writes **Spec.md** and **Build.md** into your Project Files. When both exist, this tab unlocks.' });
      touchConversation();
      renderMessages();
      setStreamingState(false);
      return;
    ...
    if (pendingImage) {
      var lastMsg = optimizedMsgs[optimizedMsgs.length - 1];
    ...
    if (content && content.trim()) {
      ...
      conv.messages.push({ role: 'assistant', content: 'I couldn\'t reach the AI backend. Ask an admin to configure the BrainStem host, then send your message again.' });
      touchConversation();
    ...
    function makeRow(node, indent) {
      ...
      if (node.type === 'file') {
        ...
        delBtn.addEventListener('click', function (e) {
          ...
          if (!node.id) return ashatToast('Disk-only file — delete it on the server.', 'warn');
  ...
  async function openFileInEditor(id, path) {
    try {
      // Load by path (server falls back to disk for files without a DB row).
      var resp = await ashatFetch('/api/files/read?path=' + encodeURIComponent(path || ''));
  ...
  function init() {
    ...
    // Image drag-drop/paste + build tab lock state
    bindImageInput();
    refreshBuildTab();
  }
