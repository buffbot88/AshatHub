...
<div class="mb-5 text-sm" style="color: var(--gold-muted);">
...
<?php else: ?>
  ...
  <input type="text" id="user-search" placeholder="Search users by name, email, or username..."
         style="width: 100%; padding: 8px 12px; background: var(--ink-soft); border: 1px solid var(--gold-line); border-radius: 6px; color: var(--gold); font-family: var(--font-mono); font-size: 13px;"
          ...
          <tr style="border-top: 1px solid var(--gold-line);" onmouseover="this.style.background='rgba(15,15,23,0.3)'" onmouseout="this.style.background=''">
            ...
            <td class="py-3 px-4 text-xs hidden sm:table-cell font-mono" style="color: var(--gold-muted);">
              ...
              <?php if (!empty($u['email_verified_at'])): ?>
                <span title="Email verified" style="color: var(--gold-ok); font-size: 10px;">✓</span>
              <?php else: ?>
                <span title="Email not verified" style="color: var(--gold-warn); font-size: 10px;">⚠</span>
              <?php endif; ?>
            </td>
            <td class="py-3 px-4">
              <?php if ($isSelf): ?>
              ...
              <?php else: ?>
                <form method="post" action="/admin/users/role/" class="flex items-center gap-1.5">
                  ...
                  <input type="hidden" name="next" value="/admin/#tab=users">
