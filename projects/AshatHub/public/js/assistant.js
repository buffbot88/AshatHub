...
(function () {
  ...
  var MODE_PROMPTS = {
    ...
    plan: {
      ...
      subtitle: 'Brainstorm · Plan · Craft specs with AI',
      prompt: null // Uses the main SYSTEM_PROMPT
    },
    build: {
      name: 'Build',
  ...
  function getModePrompt(mode) {
    if (mode === 'plan') return SYSTEM_PROMPT.content;
...
// ── Constants ────────────────────────────────────────────────────
  ...
  async function tryStream(body, headers) {
    ...
    try {
      ...
      var text = await agent.streamCompletion('/api/ai/v1/chat/completions', headers, body, {
