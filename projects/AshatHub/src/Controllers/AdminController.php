...
final class AdminController
{
    ...
    public function deleteHosting(RequestContext $ctx): void {
        $accountId = (int) $ctx->int('account_id');
        $pdo = \Core\Database::connection();
        $stmt = $pdo->prepare('SELECT domain FROM hosting_accounts WHERE id = ?');
        $stmt->execute([$accountId]);
        $account = $stmt->fetch(\PDO::FETCH_ASSOC);
        if ($account) {
            $domain = $account['domain'];
            $script = '/opt/ashat-hub/bin/deprovision-hosting.sh';
            $cmd = "sudo {$script} " . escapeshellarg($domain) . " 2>&1";
            shell_exec($cmd);
        }
    ...
    private function provisionHostingAccount(array $account): void {
        ...
        $dbName = 'host_' . $safeUserId;
        $dbUser = 'host_' . $safeUserId;
        $dbPass = bin2hex(random_bytes(16));

        // 1. Create database and user for the hosted site
        try {
            ...
            error_log("Hosting DB provision error: " . $e->getMessage());
        }

        // 2. Call the provisioning script (runs as root via sudo)
        $script = '/opt/ashat-hub/bin/provision-hosting.sh';
        $domainEsc = escapeshellarg($domain);
        $userIdEsc = escapeshellarg($userId);
        $accountIdEsc = escapeshellarg((string) $accountId);
        $cmd = "sudo {$script} {$domainEsc} {$userIdEsc} {$accountIdEsc} 2>&1";
        ...
        $encryptedPass = encryptHostingPassword($dbPass);
        $stmt = $pdo->prepare('UPDATE hosting_accounts SET status = ?, db_name = ?, db_user = ?, db_host = ?, db_password = ?, document_root = ? WHERE id = ?');
        $stmt->execute(['active', $dbName, $dbUser, 'localhost', $encryptedPass, $docRoot, $accountId]);
    }
}
